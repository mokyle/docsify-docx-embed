const DOCX_DEBUG = 0;
const DOCX_VIEWER_WIDTH = '50%';
const DOCX_VIEWER_HEIGHT = '50rem';
const DOCX_MARGIN_TOP = '2rem';
const DOCX_MARGIN_BOTTOM = '5rem';
...
! function() {
	...
	var docx_renderer = {code: renderer_func};
	...
	// Allowing Docsify to execute the script to embed DOCX
	window.$docsify.executeScript = true;

	// Linking Docsify to the DOCX plugin
	window.$docsify.plugins = [
		function(hook, vm) {
			hook.init(function() {
				verbose_log('DOCX Code Block Loader Plugin Initialized');
				verbose_log('This is a '.concat(mobile_view ? 'Mobile' : 'Non-mobile').concat(' Viewer.'));
			});
			hook.afterEach(function(html, next) {
				verbose_log('DOCX Code Block Loader Plugin After Hook Initialized');
				container_list = JSON.parse(localStorage.getItem('docx_container_list'));
				...
				if(container_list){
					...
					container_list.forEach(function(container){
						html += '\
						var options = {\
							height: "'+ DOCX_VIEWER_HEIGHT +'",\
							width: "'+ DOCX_VIEWER_WIDTH +'"\
						};\
                        var docxOptions = Object.assign(docx.defaultOptions, {useMathMLPolyfill: true});\
                        var container = document.getElementById("' + container['div_id'] + '");\
                        var xhr = new XMLHttpRequest();\
						xhr.open("GET", "'+ container['docx_location'] +'", true);\
						xhr.responseType = "blob";\
						xhr.onload = function(e) {\
							if (this.status == 200) {\
								var myBlob = this.response;\
								docx.renderAsync(myBlob, container, null, docxOptions);\
							}\
						};\
						xhr.send();\
						';
				...
				localStorage.removeItem('docx_container_list');
				next(html);
			});
		}
